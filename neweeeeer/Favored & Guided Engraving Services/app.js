...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        // --- 2. ROUTING LOGIC ---
        if (isAdmin) {
            // If the Admin is on the index or user page, send them to the Admin Panel
            if (currentPath.includes("index.html") || currentPath === "/" || currentPath.endsWith("/") || currentPath.includes("user.html")) {
                window.location.href = "admin.html"; 
                return; // Stop code execution
            }
        } else {
            // If a Normal User is on the index or admin page, send them to the User Panel
        ...
        // --- 3. UI UPDATES (If they are already on the correct page) ---
        ...
        // --- 4. GUEST PROTECTION ---
        // If a logged-out person tries to view the user or admin panel, kick them to index
        const currentPath = window.location.pathname.toLowerCase();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Also handle cases where sections might be shown from other interactions
    // (like buttons that call showSection directly)
    const originalShowSection = window.showSection;
    window.showSection = function(id) {
        if (originalShowSection) {
            originalShowSection(id);
...
// Make sure to also update the auth modal toggle to maintain active state
...
window.toggleAuth = function(type) {
    ...
    // When toggling auth modal, we're still on the same section
    // so mobile nav active state should remain
    const currentSection = getCurrentVisibleSection();
    updateMobileNavActiveState(currentSection);
};
