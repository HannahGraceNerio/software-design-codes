                ...
                <div class="order-card-enhanced" style="animation-delay: ${animationDelay}s;">
                    ...
                    <div class="order-body">
                        ...
                        <div style="margin-bottom: 10px; width: 100%;">
                            <button class="btn-outline" onclick="viewMyDesign('${o.id}')" style="width: 100%; padding: 10px; border-color: var(--primary); color: var(--primary);">
                                <i class="fas fa-eye" style="margin-right: 8px;"></i> View My Design
                            </button>
                        </div>
                        ` : ''}
                    </div>
...
window.viewMyDesign = async (orderId) => {
    try {
        // 1. Fetch the specific order data
        ...
        const proofText = document.getElementById('userPreviewText');
        const rot = data.engravingRotation || 0;
        

        proofText.style.transform = `translate(calc(-50% + ${data.posX}px), calc(-50% + ${data.posY}px)) rotate(${rot}deg)`;
        
        proofText.innerText = data.personalization || data.engravingText || "No text";
        proofText.style.fontSize = (data.engravingSize || 30) + "px";

        const font = data.engravingFont || "Arial";
        proofText.style.fontFamily = `"${font}", sans-serif`;
...
window.openDesignStudio = () => {
    ...
    const item = document.getElementById('draggableEngraving');
    
    // If there's an existing design, load it. Otherwise reset.
    if (!currentDesignData) {
        document.getElementById('studioText').value = "";
        
        // NEW: Reset the rotation slider
        const rotateSlider = document.getElementById('studioRotateSlider');
        if (rotateSlider) rotateSlider.value = 0; 
        
        item.innerText = "Type Something...";
        xOff = 0; yOff = 0;
        
        // NEW: Set initial rotation to 0
        item.style.transform = `translate(-50%, -50%) rotate(0deg)`;
    } else {
        document.getElementById('studioText').value = currentDesignData.personalization || "";
        
        // NEW: Load saved rotation
        const rotateSlider = document.getElementById('studioRotateSlider');
        if (rotateSlider) rotateSlider.value = currentDesignData.engravingRotation || 0; 
        
        updateStudioPreview();
    }
};

window.updateStudioPreview = () => {
    const text = document.getElementById('studioText').value;
    const font = document.getElementById('studioFont').value.trim() || "Arial";
    const size = document.getElementById('studioSizeSlider').value;
    
    // NEW: Get the current rotation value
    const rotateSlider = document.getElementById('studioRotateSlider');
    const rotate = rotateSlider ? rotateSlider.value : 0; 
    
    const item = document.getElementById('draggableEngraving');
    
    item.innerText = text || "Type Something...";
    item.style.fontSize = size + "px";
    
    // NEW: Update the visual label for rotation
    const rotateLabel = document.getElementById('studioRotateLabel');
    if (rotateLabel) rotateLabel.innerText = rotate + "°";

    // NEW: Apply both position AND rotation
    item.style.transform = `translate(calc(-50% + ${xOff}px), calc(-50% + ${yOff}px)) rotate(${rotate}deg)`;

    // Dynamic Font Loader
    if(font.toLowerCase() !== "arial") {
        let link = document.getElementById('studio-font-link');
        if(!link) {
            link = document.createElement('link');
...
window.saveStudioDesign = () => {
    const textInput = document.getElementById('studioText');
    const fontInput = document.getElementById('studioFont');
    const sizeSlider = document.getElementById('studioSizeSlider');
    const rotateSlider = document.getElementById('studioRotateSlider'); // Get the slider

    const text = textInput ? textInput.value.trim() : "";

    if (!text || text === "Type Something...") {
        alert("Please enter the text you want to engrave!");
        return;
    }
    ...
    currentDesignData = {
        ...
        engravingFont: fontInput ? fontInput.value : "Arial",
        engravingSize: sizeSlider ? sizeSlider.value : "30",
        engravingRotation: rotateSlider ? rotateSlider.value : 0, // NEW: Save rotation
        posX: xOff,
        posY: yOff
    };

    // Close Studio and go back to Details
    window.closeModal('designStudioModal');
    window.openModal('productDetailsModal');

    // Update button to say "Re-customize"
    const personalizeBtn = document.getElementById('personalizeBtn');
    if (personalizeBtn) {
        personalizeBtn.innerHTML = '<i class="fas fa-sync-alt"></i> Re-customize';
        personalizeBtn.style.background = "#6d4c41"; 
    }
};

function dragMove(e) {
    if (studioDragging) {
        e.preventDefault(); // Stop the background from moving

        const clientX = e.type === "touchmove" ? e.touches[0].clientX : e.clientX;
        const clientY = e.type === "touchmove" ? e.touches[0].clientY : e.clientY;

        xOff = clientX - initialX;
        yOff = clientY - initialY;

        // NEW: Get the current rotation so it stays tilted while dragging
        const rotateSlider = document.getElementById('studioRotateSlider');
        const rotate = rotateSlider ? rotateSlider.value : 0;
...
function dragStart(e) {
    ...
    // Only drag if clicking the actual text
    if (e.target.id === "draggableEngraving") {
        studioDragging = true;
    }
}
