  ...
  try {
    ...
    messagesDiv.innerHTML = `
      <div class="text-center p-lg">
        ...
        <p class="mt-md text-brown-medium">Setting up chat...</p>
      </div>
    `;
    ...
    if (conversationQuery.empty) {
      ...
      await db.collection('messages').add({
        ...
        senderId: 'system',
        text: 'Welcome to Favored & Guided Engraving support! How can we help you today?',
        timestamp: new Date().toISOString(),
        type: 'welcome'
      });
    ...
    messagesRef.onSnapshot((snapshot) => {
        ...
        messagesDiv.innerHTML = `
          <div class="message message-admin">
            <div class="flex justify-between items-start mb-xs">
              ...
              <div class="text-xs text-brown-light">Just now</div>
            </div>
            <p>Welcome to Favored & Guided Engraving support! How can we help you today?</p>
          </div>
        `;
        return;
      }
      ...
      snapshot.forEach(doc => {
        ...
        const senderName = isUser ? 'You' : 
                          (msg.senderId === 'system' ? 'System' : 'Support Agent');
    ...
    messagesDiv.innerHTML = `
      <div class="text-center p-lg">
        ...
        <h4 class="text-brown-dark mb-sm">Unable to load chat</h4>
        <p class="text-brown-medium mb-lg">${error.message}</p>
        <button class="btn btn-outline" onclick="retryChat()">
          <i class="fas fa-redo mr-sm"></i> Retry
        </button>
      </div>
    `;
  }
...
// Also update the sendUserMessage function:
async function sendUserMessage() {
  ...
  if (!currentConversationId) {
    showAlert('Creating your chat session...', 'info');
    await loadUserChat();
...
async function loadAccountChat() {
  ...
  try {
    ...
    messagesDiv.innerHTML = `
      <div class="text-center p-lg">
        <div class="loading-spinner"></div>
        <p class="mt-md text-brown-medium">Loading chat...</p>
      </div>
    `;
    ...
    if (conversationQuery.empty) {
      ...
      currentConversationId = conversationQuery.docs[0].id;
    }
    ...
    messagesRef.onSnapshot((snapshot) => {
      ...
      if (snapshot.empty) {
        messagesDiv.innerHTML = `
          <div class="message message-admin">
            ...
            <div class="text-xs text-brown-light mt-xs">Just now</div>
          </div>
        `;
        return;
      }
      ...
      snapshot.forEach(doc => {
        ...
        const time = new Date(msg.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
        const senderName = isUser ? 'You' : 
                          (msg.senderId === 'system' ? 'System' : 'Support');
    ...
    console.error('Error loading account chat:', error);
    messagesDiv.innerHTML = `
      <div class="text-center p-lg">
        <i class="fas fa-comments text-3xl text-wood-light mb-sm"></i>
        <h4 class="text-brown-dark mb-sm">Chat Error</h4>
        <p class="text-brown-medium">${error.message}</p>
        <button class="btn btn-outline mt-sm" onclick="loadAccountChat()">
          Try Again
        </button>
      </div>
    `;
...
async function debugChatSetup() {
  ...
  try {
    ...
    console.log('✅ Conversation query successful');
    console.log('Conversations found:', convQuery.size);
    
    if (convQuery.empty) {
      console.log('⚠️ No conversation found, creating...');
      const convRef = await db.collection('conversations').add({
        userId: user.uid,
      ...
      console.log('✅ Conversation exists:', convQuery.docs[0].id);
    }
...
window.debugChatSetup = debugChatSetup;  // ADD THIS LINE!
