...
window.closeModal = (id) => {
    ...
    // ALWAYS reset the Add Product form back to default when it closes
    if (id === 'addProductModal') {
        if(document.getElementById('addName')) document.getElementById('addName').value = '';
        if(document.getElementById('addDescription')) document.getElementById('addDescription').value = '';
        if(document.getElementById('addPrice')) document.getElementById('addPrice').value = '';
        if(document.getElementById('addStock')) document.getElementById('addStock').value = '10';
        
        const fileInput = document.getElementById('addImageFile');
        if (fileInput) {
            fileInput.value = ''; // Clears the internal file memory
            const uploadArea = fileInput.previousElementSibling; // Targets the visual box
            if (uploadArea) {
                // Restores the original upload icon and text
...
async function loadInventory() {
    ...
    onSnapshot(collection(db, "products"), (snapshot) => {
        ...
        products.sort((a, b) => {
            ...
            return timeA - timeB; // Swap to (timeA - timeB) if you want Oldest to Newest
        });
        ...
        snapshot.forEach(docSnap => {
            ...
            if (o.status === 'Cancelled' || o.status === 'Rejected' || o.status === 'Completed') {
                ...
                select.onchange = (e) => {
                    ...
                    document.getElementById('confirmStatusText').innerHTML = `Are you sure you want to move this order forward to <strong>"${newStatus}"</strong>?`;
                    window.openModal('confirmStatusModal');
                };
            }
...
// =========================================
window.confirmStatusUpdate = async () => {
    if (!pendingStatusUpdate) return;
    
    const { id, newStatus, productId, quantity } = pendingStatusUpdate;
    ...
    try {
        ...
        await updateOrderStatus(id, newStatus, productId, quantity);
        
        if (newStatus === 'Rejected') {
            if (typeof showToast === 'function') showToast("Order rejected and locked.", "error");
        } else if (newStatus === 'Completed') {
            if (typeof showToast === 'function') showToast("Order completed!", "success");
        } else {
        ...
        window.closeModal('confirmStatusModal');
    }
};

window.cancelStatusUpdate = () => {
    if (pendingStatusUpdate) {
        // Revert the dropdown visually back to the old status
