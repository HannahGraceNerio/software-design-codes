...
window.handleForgotPassword = async (event) => {
    ...
    // 2. Smart target: Look for either ID just in case the HTML wasn't changed
    const emailInput = document.getElementById('forgotEmail') || document.getElementById('resetEmail'); 
    
    if (!emailInput) {
        window.showToast("System error: Could not find the email box.", "error");
        return;
    }
    
    const email = emailInput.value.trim();
    const resetBtn = document.querySelector('#forgotSection .btn-auth-submit') || document.activeElement;
    const originalText = resetBtn ? resetBtn.innerHTML : 'Send Reset Link';
    ...
    // 5. Send the request to Firebase Backend
    try {
        ...
        let friendlyMessage = "Error sending reset email. Please try again.";
        
        // Translate Firebase backend errors into friendly alerts
