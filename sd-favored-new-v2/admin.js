...
window.saveProductEdit = async () => {
    ...
    // Show loading on your update button
    const saveBtn = document.querySelector('#editProductModal .btn-primary');
    saveBtn.disabled = true;

    try {
        if (file) {
            // If there's a new file, convert to base64 and update
            ...
            await updateDoc(doc(db, "products", editingProductId), { name, price, stock });
            closeEditModal();
        }
    } catch (err) {
        console.error(err);
...
function closeEditModal() {
    ...
    document.getElementById('editFileNameDisplay').innerHTML = 'Click to change image';
}
