...
window.openModal = (id) => {
    ...
    if(el) {
        // 1. Show the main modal container
        el.style.display = 'flex';
        
        // 2. FORCE reset if it's the auth modal
        if(id === 'authModal') {
            const loginPart = document.getElementById('loginSection');
            const signupPart = document.getElementById('signupSection');
            
            if (loginPart && signupPart) {
                loginPart.style.display = 'block'; // Always show login
                signupPart.style.display = 'none';  // Always hide signup
            }
        }
    }
};
